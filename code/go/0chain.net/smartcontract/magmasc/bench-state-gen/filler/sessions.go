...
import (
	...
	"0chain.net/smartcontract/magmasc/bench-state-gen/bar"
)
...
const (
	sessActivePrefix   = "act_"
	sessInactivePrefix = "ina_"
	sessNamePrefix     = "session_"
)

func getSessionName(num int, active bool) string {
	prefix := ""
	if active {
		prefix = sessActivePrefix
	} else {
		prefix = sessInactivePrefix
	}

	return prefix + sessNamePrefix + strconv.Itoa(num)
}
